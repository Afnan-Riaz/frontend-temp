...
const PediatricianSignup = ({ handleSignup }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // An array of non-required fields (optional fields)
    const nonRequiredFields = ['email']; // Only optional fields go here

    // Filtering out non-required fields and checking if any required field is empty
    ...
    try {
      ...
      const transformedData = {
        ...
        user_type: 'pediatrician',
      };

      const response = await handleSignup(transformedData);

      if (!response.success) {
        setError(response.message); // Displaying backend error message if any
      }  
    } catch (error) {
  ...
  return (
    <div className="signup-container">
      <div className="card form-container">
        ...
        <p>If already an existing pediatrician, please <Link to="/">Login</Link></p>
...
export default PediatricianSignup;
